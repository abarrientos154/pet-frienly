<template>
  <div>
    <q-card class="row q-mx-sm q-my-md justify-between bg-secondary" style="min-width: 300px">
        <div class="text-h6 q-mx-sm">Hoteleria pendientes</div>
    </q-card>
    <q-list class="row justify-center" v-if="data.length > 0">
      <q-card class="q-mx-sm q-mb-md bg-secondary col no-wrap" v-for="(item, index) in data" :key="index" v-ripple style="border-radius: 15px; max-width: 400px; min-width: 300px">
        <div class="row justify-between">
          <div class="col-4" @click="$router.push('/descripcionhotel/'+item._id)" style="width: 100px; height: 110px; border-radius: 15px">
          ...
          <div class="col-6 q-py-sm justify-center column" @click="$router.push('/descripcionusuario/'+item._id)">
            ...
            <q-chip text-color="white" label="Aprobar Hotel" color="primary" />
          </div>
    ...
    <q-card v-else class="shadow-2 q-ma-sm q-pa-md bg-secondary">
      <div class="text-center text-subtitle1">Actualmente sin Hotelerias pendientes...</div>
    </q-card>
  </div>
</template>
...
export default {
  ...
  methods: {
    getProveedores () {
      this.$api.post('user_by_statushotel', { rol: [3] }).then(res => {
        if (res) {
          this.data = res
        }
      })
    ...
    aprobarProv (id) {
      this.$q.dialog({
        title: 'Confirma',
        message: '¿Seguro deseas aprobar este Hotel?',
        cancel: true,
        persistent: true
      }).onOk(() => {
        this.$api.put('update_statushotel/' + id, { estatusHotel: 1 }).then(res => {
          if (res) {
            this.getProveedores()
          }
        })
    ...
    rechazarProv (id) {
      this.$q.dialog({
        title: 'Confirma',
        message: '¿Seguro deseas rechazar este Hotel?',
        cancel: true,
        persistent: true
      }).onOk(() => {
        this.$api.put('update_statushotel/' + id, { estatusHotel: 2 }).then(res => {
          if (res) {
            this.getProveedores()
          }
        })
